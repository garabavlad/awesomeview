    ...
    if($table_services)
    {
        ...
        foreach ($table_services as $row)
        {
            ...
            if (is_numeric($row[2])) {
                $table .= "<td>" . $row[2] . "</td>";
                $table .= "<td>
                    <button type=\"button\" id=\"mymodal\" class=\"btn btn-primary btn-awesome\" data-toggle=\"modal\" data-target=\"#anulareModal" . $row[1] . "\">
                        Book now
                    </button>
                    <!-- Modal -->
                    <form action method='POST'>
                    <input type='hidden' name='massage_id' value='". $row[1] ."'>
                    <div class=\"modal fade\" id=\"anulareModal" . $row[1] . "\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"anulareModal" . $row[1] . "Label\">
                      <div class=\"modal-dialog modal-massage-booking\" role=\"document\" style='padding-top: 10%'>
                        <div class=\"modal-content\">
                        
                          <div class=\"modal-header\">
                            <h3 class=\"modal-title\" id=\"exampleModalCenterTitle\" style='float: left;width: 89%;margin-left: 7%;'>New massage booking</h3>
                            <button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span id='massageModalClose". $row[1] ."' aria-hidden=\"true\">&times;</span></button>
                          </div>
